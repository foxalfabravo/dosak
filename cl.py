...
def cygpath(wpath):
    s1 = wpath.replace("\\","/")
    s2 = s1.replace("C:","/cygdrive/c")
    path = s2.replace("c:","/cygdrive/c")
    return path
...
def InvokeClWithDependencyGeneration(cmdline):
    ...
    depstarget = fileName + ".d"

    cmdline += ['-showIncludes']
    #print '[%s]' % ', '.join(map(str, cmdline))
...
# En francais
#        if line.startswith( 'Remarque' ):
#            dep = line[35:].strip()

# En anglais
        if line.startswith( 'Note: including' ):
            dep = line[21:].strip()
    ...
    depsdir = os.path.dirname(target)
    ...
    cygTarget = cygpath(target)
    for dep in sorted(deps):
        print >>f, "%s: %s" % (cygTarget, cygpath(dep))
